...
export default class Channels extends Component {
  ...
  render() {
    const { loading, channels, currentPage } = this.state;

    let arr = [];
    let amount = Math.round(channels.length / 25) + 1;

    for (let i = currentPage * 25; i < currentPage * 25 + 25; i++) {
      if (channels[i] !== undefined) arr.push(channels[i]);
    }
            ...
            <div className="row">
              <div
                className="col-sm-12 col-lg-8 
                        order-1 
                        order-sm-1 order-lg-1 mb-3 border-0"
              >
                <div className="mb-2">
                  <Link
                    to="/messages"
                  ...
                  <Link to="/channels" className="btn btn-outline-light mr-3">
                    Channels
                  </Link>
                </div>
              </div>
